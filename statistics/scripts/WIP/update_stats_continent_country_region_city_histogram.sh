...
declare -a query_jobs=("continent_country_region_city_histogram")
...
#  --use_legacy_sql=false "DELETE FROM \`measurement-lab.mlab_statistics.   \` WHERE test_date BETWEEN \"${startday}\" AND \"${endday}\"")
...
for val in ${query_jobs[@]}; do
  ...
  declare -a location_combos_query=("get_continent_country_region_city_codes")
  ...
  for v in ${location_combos_query[@]}; do
    ...
    "$(cat "queries/${QUERY2}")" > continent_country_region_city_codes.csv ) 
  done

  # bq exports csvs with a header. remove the header.
  sed -i '1d' continent_country_region_city_codes.csv

  # Make a temporary GCS bucket to store results.
  gsutil mb gs://temp_stats_continent_country_region_city
  ...
  for year in "${year_range[@]}"; do
    ...
    while IFS=, read -r continent country region city;
    do  
      iso_region="$country-$region"

      JOB_ID3=$(bq --nosync query \
      --use_legacy_sql=false --max_rows=4000000 --allow_large_results \
      --destination_table "mlab_statistics.temp_continent_country_region_city_stats" \
      --replace "SELECT * FROM \`mlab_statistics.continent_country_region_city_histogram\` WHERE continent_code = \"${continent}\" AND country_code = \"${country}\" AND ISO3166_2region1 = \"${iso_region}\" AND city = \"${city}\" ORDER BY test_date, continent_code, county_code, country_name, ISO3166_2region1, city, bucket_min, bucket_max, frac, samples")
      ...
      bq extract --destination_format NEWLINE_DELIMITED_JSON \
        mlab_statistics.temp_continent_country_region_city_stats \
        gs://temp_stats_continent_country_region_city/${continent}/${country}/${region}/${city}/${year}/histogram_daily_stats.json      

    done < continent_country_region_city_codes.csv

  done
  
  # Copy the full list of generated stats from measurement-lab project temp GCS bucket
  gsutil -m cp -r gs://temp_stats_continent_country_region_city/* ./tmp/
...
gsutil rm -r gs://temp_stats_continent_country_region_city

## Remove local copies.
rm -r ./tmp/*
rm continent_country_region_city_codes.csv
