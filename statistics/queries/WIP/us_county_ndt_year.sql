...
mlab_dl_perip_perday AS (
  ...
  FROM dl
  GROUP BY test_date, state, GEOID, county_name, clientIP
),
county_stats_dl AS (
  SELECT
...
JOIN county_stats_ul USING (year, state, GEOID, county_name)
JOIN county_dl_sample USING (year, state, GEOID, county_name)
JOIN county_ul_sample USING (year, state, GEOID, county_name)
JOIN counties USING (GEOID, county_name)