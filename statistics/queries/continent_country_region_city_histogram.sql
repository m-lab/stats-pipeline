...
dl_stats_per_day AS (
  SELECT
    date, continent_code, country_code, ISO3166_2region1, city,
...
results AS (
  SELECT *, MOD(ABS(FARM_FINGERPRINT(city)), 4000) as shard FROM dl_histogram
...
ORDER BY date, continent_code, country_code, ISO3166_2region1, city, bucket_min
