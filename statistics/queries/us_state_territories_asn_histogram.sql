...
dl_fingerprinted AS (
  ...
  GROUP BY date, continent_code, country_code, ISO3166_2region1, GEOID, FIPS, state, state_name, asn, ip
),
...
dl_stats_per_day AS (
  SELECT
    date, continent_code, country_code, ISO3166_2region1, GEOID, FIPS, state, state_name, asn,
  ...
  GROUP BY date, continent_code, country_code, ISO3166_2region1, GEOID, FIPS, state, state_name, asn
),
...
results AS (
  ...
  JOIN ul_histogram USING (date, continent_code, country_code, ISO3166_2region1, GEOID, FIPS, state, state_name, asn, bucket_min, bucket_max)
  JOIN dl_stats_per_day USING (date, continent_code, country_code, ISO3166_2region1, GEOID, FIPS, state, state_name, asn )
  JOIN ul_stats_per_day USING (date, continent_code, country_code, ISO3166_2region1, GEOID, FIPS, state, state_name, asn )
)
--Show the results
SELECT * FROM results
