...
--EU NUTS 3 geographies are identified for test results using a GIS approach.
-- '2016' in this filename refers to the 2016 release of NUTS geographies from
-- the European Union.
-- The lat/lon annotated on each test row is looked up in the NUTS 3 polygons
...
nuts3 AS (
  ...
  FROM
    `measurement-lab.geographies.eu_NUTS_3_2016_01m`
),
...
dl_per_location_cleaned AS (
  SELECT
    ...
    a.MinRTT AS MinRTT
  FROM `measurement-lab.ndt.unified_downloads` tests, nuts3
...
ul_per_location_cleaned AS (
  SELECT
    ...
    a.MeanThroughputMbps AS mbps,
    a.MinRTT AS MinRTT
  FROM `measurement-lab.ndt.unified_uploads`, nuts3
  WHERE date BETWEEN "2018-01-01" AND "2018-12-31" #@startdate AND @enddate
  AND a.MeanThroughputMbps != 0
  AND client.Geo.ContinentCode IS NOT NULL AND client.Geo.ContinentCode != ""
  AND client.Geo.CountryCode IS NOT NULL AND client.Geo.CountryCode != ""
