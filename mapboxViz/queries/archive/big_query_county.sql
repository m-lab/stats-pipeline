...
main AS (
  ...
  FROM
    dl
    JOIN ul USING (geo_id, time_period)
),
main_dec_2014 AS (
  SELECT
    geo_id,
...
SELECT
  ...
  ml_min_rtt_dec_2018,
  counties.geo_id as GEOID,
  counties.county_name AS county_name,
  counties.county_geom AS WKT
FROM
  `mlab-sandbox.usa_geo.us_counties` counties
  LEFT JOIN main_dec_2014 USING (geo_id)
  LEFT JOIN main_jun_2015 USING (geo_id)
  LEFT JOIN main_dec_2015 USING (geo_id)
  LEFT JOIN main_jun_2016 USING (geo_id)
  LEFT JOIN main_dec_2016 USING (geo_id)
  LEFT JOIN main_jun_2017 USING (geo_id)
  LEFT JOIN main_dec_2017 USING (geo_id)
  LEFT JOIN main_jun_2018 USING (geo_id)
  LEFT JOIN main_dec_2018 USING (geo_id);