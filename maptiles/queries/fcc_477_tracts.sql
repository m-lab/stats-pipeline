...
WITH census_blocks AS (
  ...
  FROM
    `mlab-sandbox.usa_geo.cb_2019_census_tracts`
),
dl AS (
  SELECT
    tests.*,
  ...
  WHERE
    ...
    # ? why was this server geolocation in USBB ?
    AND (
...
fccdata AS (
  ...
  UNION ALL SELECT *, 'jun_2018' AS time_period FROM `mlab-sandbox.fcc.477_jun_2018`
  UNION ALL SELECT *, 'dec_2018' AS time_period FROM `mlab-sandbox.fcc.477_dec_2018v2`
  UNION ALL SELECT *, 'jun_2019' AS time_period FROM `mlab-sandbox.fcc.477_jun_2019v1`
),
